...
2. **Round 2（互评轮）**：当前 Claude Code **不能续接已 spawn 的子代理**，所以为每个臭皮匠 **re-spawn 一个新 Opus 子代理**，prompt 注入「它上一轮答案 + 另两家信息（**digest 转自述卡、full 转完整全文**）+ 工作黑板 + 角色 + 议程」，按 spawn 顺序分派 **严谨者 / 发散者 / 批判者**。**互评议程**（不复述上轮，只做三件事：① 挑另两家的隐含假设/边界/反例 ② 把每个争议转成一个最小可验证动作 ③ 明确哪些已稳可进共识、哪些仍是候选），并要求更新「自述卡」。三个 re-spawn 同一条消息并发。等三者返回。
